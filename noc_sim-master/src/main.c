...
for (i=0; i<= numrouters; i++){
		...
		if(neighborBotom(src.x[i],src.y[y]>0).c_str()){
			fprintf(f, "\t\t\t\t\t<bottom>" + std::to_string(src.x[i]) + std::to_string(neighborBottom(src.x[i],src.y[i])) + "</bottom>\n", text);
		...
		if(neighborRight(src.x[i],src.y[y]>0).c_str()){
			fprintf(f, "\t\t\t\t\t<right>"+ std::to_string(neighborRight(src.x[i],src.y[i])) + std::to_string(src.x[i]) + "</right>\n", text);
	    ...
	    fprintf(f, "\t\t\t\t\t<local>proc" + procID + "</local>\n", text);
        fprintf(f, "\t\t\t\t</node>\n" ,text);
...
for(int i =0; i<numrouters;i++){
   ...
   fprintf(f, "\t\t\t\t<router>\n" text);
    ...
    fprintf(f, "\t\t\t\t\t<id>" + std::to_string(src.x[i]) + std::to_string(src.y[i]) + "</id>\n", text);
    fprintf(f, "\t\t\t\t\t<message_count>" + std::to_string(countMes[i]) + "<message_count>\n", text);
    fprintf(f, "\t\t\t\t\t<ports>\n", text);
    fprintf(f, "\t\t\t\t\t\t<top>" + std::to_string(percT) + "%"+ "</top>\n" , text); 
    fprintf(f, "\t\t\t\t\t\t<bottom>" + std::to_string(percB) + "%"+ "</bottom>\n" , text);  
    fprintf(f, "\t\t\t\t\t\t<left>" + std::to_string(percL) + "%"+"</left>\n" , text);    
    fprintf(f, "\t\t\t\t\t\t<right>" + std::to_string(percR) + "%" + "</right>\n" , text);   
    fprintf(f, "\t\t\t\t\t\t<local>" + std::to_string(percLoc) + "%" + "</local>\n" , text); 
    fprintf(f, "\t\t\t\t\t</ports>\n", text);
    fprintf(f, "\t\t\t\t</routers>\n" ,text);
}
...
fprintf(f, "\t\t</communication>\n", text);
fprintf(f, "\t\t<execution>" , text);

for(int i =0; i<numrouters;i++){
        fprintf(f, "\t\t\t <proc>\n", text);


        fprintf(f, "\t\t\t\t <id>" + "Proc"+std::to_string(i)+ "</id>\n", text);
        fprintf(f,"\t\t\t\t<requests>" + std::to_string(local[i]) + "</requests>\n", text);
